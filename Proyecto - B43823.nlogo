...
[
  ...
  alianzas-riqueza ;;Lista con cantidad de riqueza por alianza
  alianzas-miembros ;;Lista con cantidad de miembros por alianza
]
...
[
  ...
  id-alianza ;;ID de la alianza a la que pertenece el intermediario. Al inicio está en -1 porque no pertenece a ninguna.
...
to set-intermediarios
  ...
  ask intermediarios [
    ...
    set id-alianza -1 ;;al inicio no pertenece a ninguna alianza.
    set alianzas-riqueza [] ;;lista de alianzas.
    set alianzas-miembros [] ;;miembros por alianza.
  ]
end
...
to intermediario-pedir-ayuda-mercado-cerrado ;;Estado 2
  ...
  if-else subestado = 0[ ;;Primera vez que el intermediario entra en este estado, se ordenará la lista de conocidos según una heurística
                         ;;y se le preguntará al primero si quiere hacer un trato
...
to-report preguntar-intermediario-conocido
  ...
  if-else(indice-conocidos < (length conocidos))[ ;;Tengo conocidos a los que les puedo preguntar si quieren negociar
    ...
    let confianza (item 1 conocido-temporal)

    ask intermediario id-conocido [
      if-else estado = 2 [ ;;Está pidiendo ayuda tambien
        if-else decidir-si-negociar id-conocido confianza[ ;;Quiere negociar
  ...
  if-else not(empty? desconocidos) [
    ...
    ask intermediario conocido-intermedio[
      if-else estado = 2 [
        ...
        if-else decidir-si-negociar conocido-intermedio -1 [ ;;Quiere negociar
...
to procesar-respuesta [respuesta]
  print word "Respuesta de si quiere o no negociar " respuesta
  if (not(empty? respuesta)) [
    ...
    if-else ((item 1 respuesta) = 0) [ ;;No hubo respuesta.
      ...
      let c (item 0 respuesta)
      set conocidos lput (list c 0 []) conocidos  ;;2019 - VERIFICAR SI NO ESTABA YA EN LA LISTA, SI NO PARA QUITAR EL ANTERIOR
    ]
    [
      if-else ((item 1 respuesta) = 1) [ ;;Hubo respuesta y quiere negociar.
        ...
        ;;2019 FALTA AGREGAR LAS EXPERIENCIAS;;

        ;;Buscar en lista de conocidos
        ;;Caso 1: si esta. Solo debemos sumarle 1 a la confianza de dicho intermediario conocido.
        ;;Caso 2: no esta. Agregar el desconocido a la lista de conocidos y colocar su confianza en 0.
        let oferta-demanda buscar-coincidencia-entre-intermediario (item 0 respuesta)
        let pos-conocidos position (item 0 respuesta) map first conocidos
        if-else(pos-conocidos != false) [
          let nueva-confianza (item 1 item pos-conocidos conocidos) ;;Se aumenta la confianza.
          let id (item 0 item pos-conocidos conocidos)

          set nueva-confianza (nueva-confianza + 1) ;;Se guarda el id del intermediario.
          set conocidos remove pos-conocidos conocidos ;;Se remueve de la lista.
          set conocidos lput (list id nueva-confianza []) conocidos ;;Y se vuelve a agregar con la nueva confianza.
          ;;set conocidos lput (list id nueva-confianza (list ticks 0 )) conocidos ;;Y se vuelve a agregar con la nueva confianza.
                               ;;[id, confianza, [fecha,tipo,precio,resultado]]
                      ;;2019   ;; resultado será la comisión recibida
                               ;; tipo??
        ]
        [
          set conocidos lput(list (item 0 respuesta) 0 []) conocidos ;;desconocido pasa a ser conocido y su confianza se pone en 0.
        ]
      ...
      [
        if ((item 1 respuesta) = 2) [ ;;Hubo respuesta y no quiere negociar.
          ...
          ;;Buscar en la lista de conocidos.
          ;;Caso 1: si esta. Debemos restarle 1 a la confiaza de dicho intermediario conocido.
          ;;Caso 2: no esta. Agregarlo a la lista de conocidos pero ponerle su confianza en 0.
          let pos-conocidos position (item 0 respuesta) map first conocidos
          if-else(pos-conocidos != false) [
            let nueva-confianza (item 1 item pos-conocidos conocidos) ;;Se reduce la confianza.
            let id (item 0 item pos-conocidos conocidos)
            if(nueva-confianza > 0) [ ;;Se debe verificar que la confianza no se pase a numeros negativos pues el grado de confianza mas bajo es 0.
              set nueva-confianza (nueva-confianza - 1)
            ]
            set conocidos remove pos-conocidos conocidos ;;Se remueve de la lista.
            set conocidos lput (list id nueva-confianza []) conocidos ;;Y se vuelve a agregar con la nueva confianza.
          ]
    ...
    let negociador-conocido (item 0 respuesta)
    formar-alianza negociador-conocido
  ]
end

;;Se definira como criterio para formar alianzas que la confianza entre dos intermediarios sea mayor o igual a 2
to formar-alianza [negociador-conocido]
  print word "conocidos " conocidos
  print "ENTRE A FORMAR-ALIANZA"
  let x false
  let y false
  let posicion-en-lista -1
  let cant-miembros 0
  let pos-negociador-conocido 0
  let confianza-negociador 0

  if not(empty? conocidos) [ ;;Primero verifica que la lista de conocidos no este vacia.
    ;;set pos-negociador-conocido (position (list negociador-conocido 1 []) conocidos)
    set pos-negociador-conocido position negociador-conocido map first conocidos
    print word "posicion " pos-negociador-conocido

    ;;Primero hay que verificar que negociador-conocido este en en la lista de conocidos.
    if-else(pos-negociador-conocido != false) [ ;;Si si esta solo se accede su confianza.
      set confianza-negociador (item 1 (item pos-negociador-conocido conocidos))
      print word "confianza-negociador " confianza-negociador
    ]
    [
      set conocidos lput (list negociador-conocido 1 []) conocidos ;;Si no, se mete el negociador-conocido en conocidos.

      set pos-negociador-conocido position negociador-conocido map first conocidos ;;Se cambia el valor de pos-negociador-conocido por el nuevo valor.
      print word "posicion " pos-negociador-conocido

      set confianza-negociador (item 1 (item pos-negociador-conocido conocidos)) ;;Y se accede a la confianza.
      print word "confianza-negociador " confianza-negociador
    ]
    ;;Si la confianza del negociador-conocido es mayor o igual a el valor seleccionado en el deslizador.
    if(confianza-negociador >= nivel-confianza-alianza) [
      if-else(id-alianza = -1) [ ;;Si el intermediario aun no tiene ninguna alianza.
        ask intermediarios with [id-intermediario = negociador-conocido] [
          print word "INTERMEDIARIO " id-alianza
          ;;CASO 1: INTERMEDIARIO Y NEGOCIADOR-CONOCIDO NO PERTENECEN A UNA ALIANZA.
          if-else(id-alianza = -1) [
            print word "CASO 1: INTERMEDIARIO Y NEGOCIADOR-CONOCIDO NO PERTENECEN A UNA ALIANZA. ID NEGOCIADOR " id-alianza
            ;;Se crea la alianza y se agrega el negociador-conocido.
            set x true
            set alianzas-riqueza lput (haber-intermediario) alianzas-riqueza
            set alianzas-miembros lput 1 alianzas-miembros
            set posicion-en-lista (position haber-intermediario alianzas-riqueza)
            set id-alianza posicion-en-lista
          ]

          ;;CASO 2: INTERMEDIARIO NO PERTENECE A UNA ALIANZA PERO EL NEGOCIADOR-CONOCIDO SI.
          [
            set posicion-en-lista id-alianza ;;Guarda la posicion de negociador-conocido en alianzas-miembros.
            set cant-miembros (item posicion-en-lista alianzas-miembros) ;;Se guarda la cantidad de miembros que hay en la alianza.
            if(cant-miembros < intermediarios-por-alianza) [ ;;Si aun cabe en la alianza.
              print word "CASO 2: INTERMEDIARIO NO PERTENECE A UNA ALIANZA PERO EL NEGOCIADOR-CONOCIDO SI. ID NEGOCIADOR " id-alianza
              set y true
            ]
          ]
        ]
        if(x = true) [ ;;Entro en caso 1.
          ;;Se agrega el intermediario actual a la alianza.
          let riqueza (item posicion-en-lista alianzas-riqueza)
          set riqueza (riqueza + haber-intermediario)
          set alianzas-riqueza replace-item posicion-en-lista alianzas-riqueza (riqueza)

          set cant-miembros (item posicion-en-lista alianzas-miembros)
          set cant-miembros (cant-miembros + 1)
          set alianzas-miembros replace-item posicion-en-lista alianzas-miembros(cant-miembros)

          set id-alianza posicion-en-lista
          set x false ;;Se resetea.
        ]
        if(y = true) [ ;;Entro en caso 2.
          let riqueza (item posicion-en-lista alianzas-riqueza)
          set riqueza (riqueza + haber-intermediario)
          set alianzas-riqueza replace-item posicion-en-lista alianzas-riqueza (riqueza)
          ...
          set id-alianza posicion-en-lista
          set y false ;;Se resetea.
        ]
      ]
      [
        ;;CASO 3: INTERMEDIARIO YA PERTENECE A UNA ALIANZA PERO EL NEGOCIADOR-CONOCIDO NO.
        if(id-alianza != -1) [ ;;Si el intermediario ya tiene una alianza.
          print word "INTERMEDIARIO " id-alianza
          set posicion-en-lista id-alianza ;;Guarda la posicion de intermediario en alianzas-miembros.
          set cant-miembros (item posicion-en-lista alianzas-miembros) ;;Se guarda la cantidad de miembros que hay en la alianza.
          ask intermediarios with [id-intermediario = negociador-conocido] [
            if(id-alianza = -1) [ ;;Si el negociador-conocido no tiene ninguna alianza.
              if(cant-miembros < intermediarios-por-alianza) [ ;;Si aun cabe en la alianza.
                print word "CASO 3: INTERMEDIARIO YA PERTENECE A UNA ALIANZA PERO EL NEGOCIADOR-CONOCIDO NO. ID NEGOCIADOR " id-alianza
                let riqueza (item posicion-en-lista alianzas-riqueza)
                set riqueza (riqueza + haber-intermediario)
                set alianzas-riqueza replace-item posicion-en-lista alianzas-riqueza (riqueza)
          ...
          ;;NOTA: si ya ambos pertenecen a una alianza diferente entonces no pueden formar una entre ellos.
        ]
      ]
    ]
  ]
  print word "LISTA ALIANZAS-CANT-MIEMBROS " alianzas-miembros
  print word "LISTA ALIANZAS-RIQUEZA " alianzas-riqueza
end
...
to vender-accion [oferta-demanda] ;;oferta y demanda tiene el formato [[id-inter [oferta]] [id-otro-inter [demanda]]].
  ...
  if-else id-intermediario = id-intermediario1 [ ;;Yo puse la oferta
    ...
    ;;Actualizar lista conocidos para agregarle esta experiencia
    let temp-conocido item id-intermediario1 conocidos
    show temp-conocido
    stop
    ;set conocidos replace-item id-intermediario1 conocidos
...
;; Es necesario hacer una heuristica que mande como parámetro el
;; id de un agente y a partir de quien quiera negociar conmigo y
;; las experiencias que he tenido con esa persona, decidir si negociar o no
;;-------  TO DO ------;;-------  TO DO ------;;-------  TO DO ------;;-------  TO DO ------;;-------  TO DO ------
to-report decidir-si-negociar [id-agente-conocido confianza]
  print "Estamos en decidir-si-negociar"
  print word "Confianza agente conocido " confianza

  ifelse confianza > 0
  [
    let numero-random random 101
    if-else (numero-random <= (probabilidad-negociar + 5 * confianza))[report true][report false]
  ]
  [
    let numero-random random 101
...
to-report buscar-coincidencia-entre-intermediario [id-segundo-intermediario]
  ...
  let ids-coincidencia [] ;;Aquí se pondran las coincidencias con ofertas y demandas con id respectivo [[id , oferta] [id , demanda]]
  ...
  ask intermediario id-segundo-intermediario [
    ...
    if-else (empty? coincidencia-oferta-demanda) [;;No hubo coincidencias
      ...
      if-else (empty? coincidencia-oferta-demanda) [;;No hubo coincidencias tampoco, no hay nada que hacer
...
"" ""
PENS
"default" 1.0 0 -955883 true "" "if not(empty? alianzas-riqueza)[\n  plot mean alianzas-riqueza\n]"
